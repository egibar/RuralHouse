...
public class BookRuralHouseGUI extends JFrame {
	...
	public BookRuralHouseGUI(int houseNumber, Date firstDay, Date lastDay) {
		try {
			...
			// Put the "houseNumber", "firstDay" and "lastDay" in the graphical
			...
			c.setTime(firstDay);
			// jCalendar1.setCalendar(new
			// GregorianCalendar(firstDay.getYear()+1900,firstDay.getMonth(),firstDay.getDate()));
			jCalendar1.setCalendar(c);

			long numberOfDays = (long) (lastDay.getTime() - firstDay.getTime())
					/ MILLSECS_PER_DAY;
	...
	private void jbInit() throws Exception {
		...
		jButton2.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				// Arrival date
				Date firstDay = trim(new Date(jCalendar1.getCalendar()
						.getTime().getTime()));

				System.out.println("firstDay=" + firstDay);
				// Last day
				// Number of days expressed in milliseconds
				long nights = 1000 * 60 * 60 * 24
				...
				try {
					...
					ApplicationFacadeInterface facade = StartWindow
							.getBusinessLogic();
					User u = StartWindow.getLogin();
					if (u != null) {
						Booking book = facade.createBooking(house, firstDay,
								lastDay, telephone, StartWindow.getLogin());
						if (book != null) {
						...
						jLabel5.setText("Error: You must be registered");
					}
